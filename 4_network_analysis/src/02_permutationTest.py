...
@author: Gehad Youssef  based on Han et al., 2021, original code created by woochanghwang.
...
def analysis_centrality_RWR(input):
    ...
    random_start_genes = random.choices(list(random_G.nodes()), k=no_membrane_nodes) # in paper: number of DIPs
    random_target_genes = random.choices(list(random_G.nodes()), k=no_nucleus_nodes) # in paper: number of DEPs
    betweensub_centrality_dict = nx.betweenness_centrality_subset(random_G, sources=random_start_genes,
    ...
    start_genes_for_PR = dict.fromkeys(random_start_genes,1)
    rwrsub_dict = nx.pagerank(random_G, personalization=start_genes_for_PR)
    # print(rwr_dict)
